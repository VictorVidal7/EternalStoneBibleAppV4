/**
 * 📖 MemoryVersePickerSheet — visual book → chapter → verse(s) picker for
 * adding cards to the memorize deck WITHOUT leaving Memorizar.
 *
 * Before this, the only ways to add a verse to the deck were (a) favoriting
 * it elsewhere and toggling the school icon in Favoritos, or (b) the
 * Bible reader. `memory/index.tsx` had no "add" affordance of its own.
 *
 * Adapted from [[SermonVersePickerSheet]] (same three-step visual pick flow,
 * same multi-select verse step) but decoupled from sermon-notes: this sheet
 * hands back the actual `BibleVerse[]` the caller selected (book/chapter/
 * verse/text) instead of formatted reference labels, so the caller can feed
 * them straight into `MemoryDeckContext`'s `addCard`.
 *
...
interface Props {
  visible: boolean;
  onClose: () => void;
  /** The selected verses, ready for MemoryDeckContext's `addCard`. */
  onAddVerses: (verses: BibleVerse[]) => void;
}

export const MemoryVersePickerSheet: React.FC<Props> = ({
  visible,
  onClose,
  onAddVerses,
}) => {
  ...
  const p = t.memory.picker;
  const tb = t.bible;
  // Book names follow the active reading version's language, same as the
  // rest of the memorize screens (practice.tsx's displayBookName).
  const bookLang = selectedVersion.language;
  ...
  const handleConfirm = useCallback(() => {
    ...
    haptics.tap();
    const picked = verses.filter(v => selectedVerses.has(v.verse));
    onAddVerses(picked);
  }, [selectedVerses, verses, onAddVerses]);

  const addLabel = useMemo(() => {
    const n = selectedVerses.size;
    if (n === 0) return p.addButtonNone;
    if (n === 1) return p.addButtonOne;
    return p.addButton.replace('{{n}}', String(n));
  }, [selectedVerses.size, p]);
            ...
            <>
              <View style={[styles.searchRow, {borderColor: colors.border}]}>
                ...
                <TextInput
                  ...
                  accessibilityLabel={p.bookSearchPlaceholder}
                />
              </View>
...
export default MemoryVersePickerSheet;
