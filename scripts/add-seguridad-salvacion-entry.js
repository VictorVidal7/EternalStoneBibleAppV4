/**
 * 📖 add-seguridad-salvacion-entry — splice the "SEGURIDAD DE LA SALVACIÓN"
 * (assurance of salvation: Arminian/Wesleyan vs Reformed) multi-view
 * v2-doctrinal dictionary entry into BOTH bundled dictionary assets, at the
 * correct sorted position and in byte-identical formatting to what
 * `scripts/build-dictionary-v2-es.js` + `scripts/split-dictionary-views.js`
 * would produce — so `git diff` shows ONLY the added entry.
 *
 * Copy of the ELECCIÓN precedent (`scripts/add-eleccion-entry.js`, see
 * DOCS/INTEGRATION-eleccion.md for the full rationale) adapted for this
 * entry. That script is hardcoded to the "eleccion" slug/paths, not
 * parameterized, so it is copied rather than repurposed in place.
 *
 * IDEMPOTENT: an existing "seguridad-salvacion" entry in either file is
 * REPLACED, not duplicated. Re-run freely after editing the section text.
 *
 * ── Usage ───────────────────────────────────────────────────────────────
 *   node scripts/add-seguridad-salvacion-entry.js
 *       Reads the clean, committed input
 *       `scripts/seguridad-salvacion-entry.json`, validates it, and splices
 *       it into both asset files.
 *
 *   node scripts/add-seguridad-salvacion-entry.js --from-draft
 *       (Re)generates `scripts/seguridad-salvacion-entry.json` by parsing
 *       the 4 sections + gloss out of
 *       `DOCS/drafts/seguridad-salvacion-integrated-draft.md` (converting
 *       prettier's `_italic_` back to the app parser's `*italic*`), then
 *       splices.
 *
 *   node scripts/add-seguridad-salvacion-entry.js --check
 *       Validate the input + report where the entry sorts, write nothing.
 *
...
const ENTRY_JSON = path.join(ROOT, 'scripts', 'seguridad-salvacion-entry.json');
const DRAFT_MD = path.join(
  ROOT,
  'DOCS',
  'drafts',
  'seguridad-salvacion-integrated-draft.md',
);
...
const SLUG = 'seguridad-salvacion';
const HEADWORD = 'SEGURIDAD DE LA SALVACIÓN';

// Short display labels for the section chips/tabs, matching the ELECCIÓN
// precedent's style and Victor's decision (neutral tradition names, not
// "apostasía" / "perseverancia").
//
// `expectInTitle` guards against a `--from-draft` MISLABEL: if the draft
// `.md` is reordered, the heading's quoted title must still contain this
// keyword or the run fails loud instead of attaching e.g. "Arminiana y
// wesleyana" to the Reformed body.
