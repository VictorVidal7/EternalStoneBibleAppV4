 ...
 * (src/lib/database/bible-data-web.ts, Sprint 66). The text is fetched + emitted
 * by scripts/fetch-web-data.js; this locks the invariants the runtime SQLite
 * loader and the reader rely on, so a future re-generation can't silently ship
 * a broken or mis-versified WEB:
...
describe('bible-data-web (WEB) integrity', () => {
  it('is a non-empty array of ~31k verses, all tagged WEB', () => {
    ...
    // The fetch produced 31,095 (WEB combines/splits a handful of verses vs the
    // 31,102 of KJV/RVR). Lock the exact count so a partial re-fetch is caught.
    expect(WEB_DATA.length).toBe(31095);
  ...
  it('contains genuine WEB wording (not a relabelled KJV)', () => {
    ...
    expect(john316?.text).toMatch(/one and only Son/);
  });
});
